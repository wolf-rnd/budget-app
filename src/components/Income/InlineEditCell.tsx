...
interface InlineEditCellProps {
  income: Income;
  field: 'name' | 'amount' | 'source' | 'note';
  value: string | number;
  type?: 'text' | 'number';
  inlineEdit: InlineEditState;
  onStartEdit: (incomeId: string, field: 'name' | 'amount' | 'source' | 'note', currentValue: string) => void;
  onSaveEdit: () => void;
  onCancelEdit: () => void;
...
const InlineEditCell: React.FC<InlineEditCellProps> = ({
  ...
  const isEditing = inlineEdit.incomeId === income.id && inlineEdit.field === field;
  ...
  return (
    <div
      onDoubleClick={() => onStartEdit(income.id, field, String(value))}
